...
- Reuse MCP server config from gemini-extension.json when available.
"""
...
DEFAULT_MCP_SERVER_NAME = "modelscope-mcp"
DEFAULT_MCP_CONFIG = {"command": "uvx", "args": ["modelscope-mcp-server"]}
...
def normalize_mcp_server_config(server_cfg: dict) -> dict:
    """Normalize a Gemini server config for Cursor .mcp.json output."""
    if "url" in server_cfg and isinstance(server_cfg["url"], str) and server_cfg["url"].strip():
        return {"url": server_cfg["url"].strip()}

    if "httpUrl" in server_cfg and isinstance(server_cfg["httpUrl"], str) and server_cfg["httpUrl"].strip():
        return {"url": server_cfg["httpUrl"].strip()}

    command = server_cfg.get("command")
    if isinstance(command, str) and command.strip():
        normalized: dict = {"command": command.strip()}
...
def extract_mcp_from_gemini() -> tuple[str, dict]:
    """Return (server_name, server_config) from gemini-extension when available."""
    if not GEMINI_EXTENSION.exists():
        return DEFAULT_MCP_SERVER_NAME, dict(DEFAULT_MCP_CONFIG)
    ...
    return server_name, normalize_mcp_server_config(server_cfg)


def build_mcp_config() -> dict:
    server_name, server_cfg = extract_mcp_from_gemini()
    return {
        "mcpServers": {
