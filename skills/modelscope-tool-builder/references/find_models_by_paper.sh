...
# Find models associated with papers on ModelScope
# Usage: ./find_models_by_paper.sh [arXiv_id|search_term]
# Optional: Set MODELSCOPE_API_TOKEN environment variable for private/gated models
...
# Help function
show_help() {
    echo -e "${BLUE}Find models associated with papers on ModelScope${NC}"
    echo ""
    ...
    echo "  --token   Use MODELSCOPE_API_TOKEN environment variable (if set)"
    echo ""
    echo -e "${YELLOW}Environment:${NC}"
    echo "  MODELSCOPE_API_TOKEN  Optional: ModelScope token for private/gated models"
    echo ""
    echo -e "${YELLOW}Examples:${NC}"
    echo "  $0 1910.01108                    # Search by arXiv ID"
    echo "  $0 distilbert                     # Search by model name"
    echo "  $0 transformer                    # Search by keyword"
    echo "  MODELSCOPE_API_TOKEN=your_token $0 1910.01108  # Use authentication"
    echo ""
    echo -e "${YELLOW}Description:${NC}"
    echo "This script finds ModelScope models that are associated with research papers."
    echo "It searches for models that have arXiv IDs in their tags or mentions papers in their metadata."
    echo ""
    echo -e "${YELLOW}Notes:${NC}"
    echo "• MODELSCOPE_API_TOKEN is optional for public models"
    echo "• Use MODELSCOPE_API_TOKEN for private repositories or gated models"
    echo "• MODELSCOPE_API_TOKEN enables higher rate limits for heavy usage"
}
...
# Set up authentication header if MODELSCOPE_API_TOKEN is available
if [[ -n "$MODELSCOPE_API_TOKEN" ]] && [[ "$USE_TOKEN" == true || -n "$MODELSCOPE_API_TOKEN" ]]; then
    AUTH_HEADER="-H \"Authorization: Bearer $MODELSCOPE_API_TOKEN\""
    echo -e "${BLUE}Using MODELSCOPE_API_TOKEN for authentication${NC}"
else
    AUTH_HEADER=""
    if [[ -n "$MODELSCOPE_API_TOKEN" ]]; then
        echo -e "${YELLOW}MODELSCOPE_API_TOKEN found but not using it (add --token flag to use)${NC}"
    fi
fi
...
get_paper_title() {
    local arxiv_id="$1"
    # Try to get paper title from ModelScope tags if available
    # This is a simplified approach - in practice, you might want to call arXiv API
    echo "Paper Title (arXiv:$arxiv_id)"
}

# Search for models
API_URL="https://modelscope.cn/api/models"
echo -e "${YELLOW}Searching ModelScope API...${NC}"
...
if [[ -n "$MODELSCOPE_API_TOKEN" ]]; then
    RESPONSE=$(curl -s -H "Authorization: Bearer $MODELSCOPE_API_TOKEN" "$API_URL?search=$SEARCH_QUERY&limit=50" || true)
else
...
echo "• Use MODELSCOPE_API_TOKEN for private models or higher rate limits"
echo ""
